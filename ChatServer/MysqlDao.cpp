...
bool MysqlDao::GetFriendList(int self_id, std::vector<std::shared_ptr<UserInfo>>& user_info_list) {
	...
	try {
		...
		while (res->next()) {
			...
			auto friend_info = GetUser(friend_id);
			if (friend_info == nullptr) {
				continue;
			}
			friend_info->back = friend_info->name;
...
bool MysqlDao::GetUserThreads(int64_t userId,
	int64_t lastId,
	int      pageSize,
	...
	try {
		std::string sql =
			...
			" ORDER BY thread_id "
			" LIMIT ?;";
		std::unique_ptr<sql::PreparedStatement>pstmt(con->con_->prepareStatement(sql));
		...
		//如果还有数据，更新nextLastId为最后一条thread_id
		if (!tmp.empty()) {
